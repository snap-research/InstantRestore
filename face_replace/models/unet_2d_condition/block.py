...
def apply_freeu(
    ...
    """
    dtype = res_hidden_states.dtype
    if resolution_idx == 0:
        num_half_channels = hidden_states.shape[1] // 2
        hidden_states[:, :num_half_channels] = hidden_states[:, :num_half_channels] * freeu_kwargs["b1"]
        res_hidden_states = fourier_filter(res_hidden_states.float(), threshold=1, scale=freeu_kwargs["s1"]).to(dtype)
    if resolution_idx == 1:
        num_half_channels = hidden_states.shape[1] // 2
        hidden_states[:, :num_half_channels] = hidden_states[:, :num_half_channels] * freeu_kwargs["b2"]
        res_hidden_states = fourier_filter(res_hidden_states.float(), threshold=1, scale=freeu_kwargs["s2"]).to(dtype)
