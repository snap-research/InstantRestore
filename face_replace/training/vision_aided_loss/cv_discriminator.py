...
from face_replace.training.vision_aided_loss.cvmodel import CVBackbone
from face_replace.training.vision_aided_loss.blurpool import BlurPool
from face_replace.training.vision_aided_loss.cv_losses import losses_list
...
class Discriminator(torch.nn.Module):
    def __init__(self, cv_type, output_type='conv_multi_level', loss_type=None, diffaug=True, device='cpu', create_optim=False, num_classes=0, activation=nn.LeakyReLU(0.2, inplace=True), **kwargs):
        ...
        def get_decoder(cv_type, output_type):
            ...
            if 'dinov2' in cv_type:
                if 'conv_multi_level' in output_type:
                    decoder = MultiLevelDViT(level=3, in_ch1=1024, in_ch2=1024, out_ch=256, down=2, num_classes=num_classes, activation=activation)
                else:
