...
class y_dequantize(nn.Module):
    ...
    def forward(self, image):
        self.y_table = self.y_table.to(image.device)
...
class idct_8x8(nn.Module):
    ...
    def forward(self, image):
        self.tensor = self.tensor.to(image.device)
        self.alpha = self.alpha.to(image.device)
...
class ycbcr_to_rgb_jpeg(nn.Module):
    ...
    def forward(self, image):
        self.shift = self.shift.to(image.device)
        self.matrix = self.matrix.to(image.device)
